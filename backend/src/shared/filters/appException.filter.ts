 ...
 * and never think about status codes.
 */
@Injectable()
@Catch()
export class AppExceptionFilter implements ExceptionFilter {
  constructor(private readonly logger: Logger) {}
    ...
    if (error.httpStatus >= 500) {
      ...
      this.logger.warn(logPayload, error.message);
    }
  ...
  private normalize(exception: unknown): AppError {
    ...
    if (exception instanceof HttpException) {
      return new HttpExceptionAdapter(exception);
    }
...
/** `getZodError()` is typed as unknown, so the issue list is narrowed by hand. */
