...
import { type AuthenticatedUser, CurrentUser, RequirePermission } from '@/shared/decorators/auth.decorators';
...
import { ChangeStateDto, CreateSurfaceObjectDto, DeleteSurfaceObjectQueryDto, KindPolicyResponseDto, SurfaceObjectResponseDto, UpdateSurfaceObjectDto } from '../dto/surfaceObject.dto';
...
export class SurfaceObjectsController {
  ...
  kinds(): readonly KindPolicyDto[] {
    return allKindPolicies().map((policy) => ({ kind: policy.kind, valence: policy.valence, allowSelfSubject: policy.allowSelfSubject, spawnRadius: policy.spawnRadius, minSeparation: policy.minSeparation }));
  }
  ...
  @ApiOkResponse({ type: SurfaceObjectResponseDto })
  async changeState(@CurrentUser() user: AuthenticatedUser, @Param('objectId') objectId: string, @Body() body: ChangeStateDto): Promise<SurfaceObjectDto> {
    return toSurfaceObjectDto(await this.changeStateHandler.execute({ objectId: objectId as SurfaceObjectId, actorUserId: user.userId, transition: body.transition, expectedVersion: body.version }));
  }

  @Patch('surface-objects/:objectId')
  @ApiOperation({ summary: 'Изменить заметку или избранное' })
  @ApiOkResponse({ type: SurfaceObjectResponseDto })
  async update(@CurrentUser() user: AuthenticatedUser, @Param('objectId') objectId: string, @Body() body: UpdateSurfaceObjectDto): Promise<SurfaceObjectDto> {
    return toSurfaceObjectDto(await this.updateHandler.execute({ objectId: objectId as SurfaceObjectId, actorUserId: user.userId, metadata: body.metadata ?? null, favorite: body.favorite ?? null, expectedVersion: body.version }));
  }

  @Delete('surface-objects/:objectId')
  @HttpCode(204)
  @ApiOperation({ summary: 'Убрать объект с поверхности' })
  async remove(@CurrentUser() user: AuthenticatedUser, @Param('objectId') objectId: string, @Query() query: DeleteSurfaceObjectQueryDto): Promise<void> {
    await this.updateHandler.delete({ objectId: objectId as SurfaceObjectId, actorUserId: user.userId, expectedVersion: query.version });
  }
}
