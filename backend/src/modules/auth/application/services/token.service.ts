...
/**
 * Access tokens are stateless JWTs; refresh tokens are opaque random strings kept
 * only as a hash. A stolen database therefore cannot mint sessions, and refresh
 * rotation can invalidate a single device.
 */
@Injectable()
export class TokenService {
  ...
  async issueAccessToken(payload: AccessTokenPayload): Promise<{ token: string; expiresAt: Date }> {
    ...
    return {
      token,
      expiresAt: new Date(Date.now() + this.config.auth.accessTtlSeconds * 1_000),
    };
  }
  ...
  createRefreshToken(sessionId: SessionId): string {
    // The session id travels with the token so refresh needs no extra lookup key.
  ...
  parseRefreshToken(token: string): { sessionId: SessionId; hash: string } {
    ...
    if (separator <= 0) {
      throw new AuthenticationError('Некорректный refresh-токен');
    }
