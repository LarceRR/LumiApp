...
import { CreateSpaceDto, InvitationResponseDto, InviteMemberDto, RespondInvitationDto, SpaceResponseDto } from '../dto/space.dto';
...
export class SpacesController {
  ...
  async list(@CurrentUser() user: AuthenticatedUser): Promise<readonly SpaceDto[]> {
    ...
    const profiles = await this.loadProfiles(spaces.flatMap((space) => space.members.map((member) => member.userId)));
