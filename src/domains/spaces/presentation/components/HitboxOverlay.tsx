  ...
  return (
    <View style={[styles.label, { left, top }]}>
      ...
      <Text
        variant="caption"
        color={HITBOX_COLOR}
      >{`центр ${round(centerX)}, ${round(centerY)} · ${round(width)}×${round(height)} px`}</Text>
      <Text variant="caption" color={HITBOX_COLOR}>{`левый верх ${round(x)}, ${round(y)}`}</Text>
    </View>
  );
}
function HitboxOverlayComponent(): ReactElement | null {
  ...
  const freeZoneHeight = Math.max(0, viewport.height - measuredSheetHeight);
  const freeZoneCenter = { x: viewport.width / 2, y: freeZoneHeight / 2 };
  ...
  return (
    <View pointerEvents="none" style={StyleSheet.absoluteFill}>
      <View style={[styles.freeZone, { width: viewport.width, height: freeZoneHeight }]} />
      <View
        style={[
          styles.box,
