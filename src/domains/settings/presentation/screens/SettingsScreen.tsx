...
export function SettingsScreen(): ReactElement {
  ...
  return (
    <Screen title="Настройки" reserveTabBar={false}>
      ...
      <BlurCard title="Отладка"><ListRow title="Показывать FPS" subtitle="Частота кадров поверх сцены" trailing={<Switch value={showPerformanceOverlay} onValueChange={setShowPerformanceOverlay} accessibilityLabel="Показывать FPS" />} /><Divider /><ListRow title="Отображение 2D хитбокса" subtitle="Снимок габаритов выбранного объекта на экране в момент нажатия" trailing={<Switch value={showHitbox} onValueChange={setShowHitbox} accessibilityLabel="Отображение 2D хитбокса" />} /><Divider /><Text variant="caption">Качество: {quality.tier}, до {quality.maxInstancesPerKind} объектов в кадре. Версия: {env.mode}</Text></BlurCard>
