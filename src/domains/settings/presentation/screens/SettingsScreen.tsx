  ...
  return (
    <Screen title="Настройки" reserveTabBar={false}>
      ...
      <BlurCard title="Сцена">
        <ListRow
          title="Фон под тему"
          subtitle="Поверхность меняет палитру вместе с интерфейсом"
          trailing={
            <Switch
              value={backgroundFollowsTheme}
              onValueChange={(value) => setSurfaceBackground(value ? null : resolvedBackground)}
          ...
          {objects.map((definition, index) => (
            <Fragment key={definition.kind}>
              ...
              <ListRow
                ...
                onPress={() => setTunedKind(definition.kind)}
              />
            </Fragment>
          ))}
        </BlurCard>
