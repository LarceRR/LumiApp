...
export type BottomBarProps = BottomTabBarProps & {
  readonly renderIcon?: (params: BottomBarIconParams) => ReactNode;
};
          ...
          {state.routes.map((route, index) => {
            ...
            const icon =
              renderIcon === undefined
                ? null
                : (renderIcon({ routeName: route.name, focused, color, size: ICON_SIZE }) ?? null);
            return (
              <TabItem
                key={route.key}
                focused={focused}
...
const styles = StyleSheet.create({
  wrap: { position: 'absolute', left: layout.tabBarInset, right: layout.tabBarInset, bottom: 0 },
  bar: { minHeight: layout.tabBarHeight },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
