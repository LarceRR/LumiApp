...
export type BottomSheetProps = {
  ...
  readonly autoFocusFirstInput?: boolean;
};
export const BottomSheet = forwardRef<BottomSheetRef, BottomSheetProps>(function BottomSheet(
  ...
  const baseHeight = Math.max(220, height * (heightFraction ?? 0.56));
  const sheetHeight = Math.max(baseHeight, contentHeight + insets.bottom + spacing.lg + spacing.xl);
  useImperativeHandle(ref, () => sheetRef.current as BottomSheetRef, []);
  useEffect(() => {
    if (visible) {
      sheetRef.current?.show();
      onSheetLayout?.(sheetHeight);
    } else sheetRef.current?.hide();
  }, [onClose, onSheetLayout, sheetHeight, visible]);
  return (
    <Sheet
      ref={sheetRef}
      height={sheetHeight}
      ...
      <KeyboardAvoidingView
        ...
        onLayout={(event) => {
          ...
          if (next !== contentHeight) {
            ...
            onSheetLayout?.(Math.max(sheetHeight, next));
          }
        }}
      >
