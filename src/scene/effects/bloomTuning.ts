...
export function bloomMipWeights(radius: number, mips: number): readonly number[] {
  const blend = clamp01(radius); const count = Math.max(1, Math.min(Math.floor(mips), BLOOM_MAX_MIPS)); const raw: number[] = [];
  for (let index = 0; index < count; index += 1) { const tight = 1 - index / BLOOM_MAX_MIPS; raw.push(tight * (1 - blend) + (1.2 - tight) * blend); }
